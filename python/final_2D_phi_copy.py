...
phis = np.linspace(0.0, 2.0, 100)
temp = np.linspace(0.1, 150, 100)
...
for j, q in enumerate(quants):
    ...
    if q == 'mag_abs':
        ...
        a = len(value_matrix[0])
        for i in range(len(value_matrix)):
            b.append(value_matrix[i][index])
            bstd.append(std_matrix[i][index])

        plt.plot(np.linspace(0, 2, len(b)), b)
        plt.plot(np.linspace(0, 2, len(bstd)), np.array(bstd)*20.0, color='red')
        plt.ylim([0, 1])
        plt.show()

        change = [1.12, 1.09, 1.09, 1.07, 1.06, 1.07, 1.02, 1.09, 1.04, 1.04, 1.07, 1.07]
        print 'mean', np.mean(change), np.std(change)
        sys.exit(0)
